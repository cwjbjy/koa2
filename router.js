...
router.post('/add',async(ctx,next)=>{
	...
	await mysql.query(`INSERT INTO USER (user_name,password,authority) VALUES ('${user_name}',MD5('${password}'),'${authority}');`);
	ctx.body={
		code:200,
	}
})
...
router.put('/update',async(ctx,next)=>{
	...
	await  mysql.query(`UPDATE USER SET user_name='${user_name}', authority=${authority} WHERE id=${id};`)
	ctx.body={
		code:200,
	}
})
		...
		Data.map(item=>{
			if(item.password == md5_password){
				if(userName == '一叶扁舟'){
					...
					ctx.body={
						auth:["firstItem","fleet","fileUp","baseEcharts","baseTable","flowChart","magnifying","drag","I18n","chatRoom"],
						value:'di5j8fy85vSAX88U'
					}
				}
				ctx.status=200
...
router.post('/register',async(ctx)=>{
	...
	let Data = JSON.parse(JSON.stringify(data))
	if(Data.length == 0){
		await mysql.query(`INSERT INTO USER (user_name,password,authority,createTime,photo) VALUES ('${userName}',MD5('${passWord}'),'${authority}','${createTime}','${photo}');`);
		ctx.body={
			message:"注册成功"
		}
	}else{
...
router.post('/uploadImage',koaBody,async(ctx)=>{
	...
	if(authorization == 'di5j8fy85vSAX88U'){
		let {user_name} = ctx.request.body;
		await mysql.query(`UPDATE USER SET photo='${newfileName}' WHERE user_name='${user_name}';`);
		ctx.body={
			code:200,
			message:"上传成功"
		}
...
router.get('/getImage',async(ctx)=>{
	let {user_name} = ctx.query;
	let data = await mysql.query(`SELECT photo FROM USER WHERE user_name='${user_name}';`);
	let Data = JSON.parse(JSON.stringify(data))
	ctx.body={
		Data,
		code:200,
